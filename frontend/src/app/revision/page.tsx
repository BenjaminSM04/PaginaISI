...
import { AlertTriangle, Check, ChevronDown, ChevronUp, ExternalLink, FileText, Github, Loader2, Rocket, X } from 'lucide-react';
      ...
      {(data ?? []).map((item) => (
        <div key={item.id} className="space-y-3 rounded-xl border border-amber-500/30 bg-card p-5 shadow-sm">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              <Check /> Aprobar {kind === 'projects' ? '(+40 pts al autor)' : '(+35 pts al autor)'}
            </Button>
            <Button
              size="sm"
              variant="outline"
