...
import {
  ArrowRight, BookOpen, Building2, Calendar, ExternalLink, Eye, Github, GraduationCap,
  Layers, Newspaper, UserPlus, Users, Video,
} from 'lucide-react';
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-3">
      <div className="space-y-6 lg:col-span-2">
          ...
          <div className="grid grid-cols-3 gap-3">
            {project.gallery!.map((image, index) => (
              <img
                key={image.id}
                src={image.url}
                alt={`${project.title} — imagen ${index + 1} de la galería`}
        ...
        {project.isIncubator && (project.clients?.length ?? 0) > 0 && (
          <section className="rounded-xl border border-purple-500/20 bg-card p-6 shadow-sm" aria-labelledby="project-clients-title">
            <div>
              <span className="section-kicker">Alianzas de la incubadora</span>
              <h2 id="project-clients-title" className="mt-1 flex items-center gap-2 font-serif-heading text-xl font-bold text-primary">
                <Building2 className="h-5 w-5 text-purple-500" /> Nuestros clientes
              </h2>
            </div>
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {project.clients!.map((client) => (
                <div key={client.id} className="flex min-w-0 items-center gap-3 rounded-xl border border-border bg-secondary/30 p-3">
                  <IncubatorClientLogo client={client} />
                  <span className="min-w-0 truncate text-sm font-bold">{client.name}</span>
                </div>
              ))}
            </div>
          </section>
        ...
        {!preview && (project.news?.length ?? 0) > 0 && (
        ...
        {preview ? (
          ...
          <>
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="ml-auto"><ReportButton targetType="PROJECT" targetId={project.id} /></div>
            </div>
      ...
      <aside className="space-y-5">
        <div className="space-y-2.5 rounded-xl border border-border bg-card p-5 shadow-sm">
          {!preview && <ProjectManageShortcut projectId={project.id} />}
          {project.repoUrl && (
        ...
        <div className="space-y-3 rounded-xl border border-border bg-card p-5 text-sm shadow-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            {preview ? `Versión ${project.pendingVersion?.number ?? 'pendiente'}` : `Publicado: ${formatDate(project.publishedAt)}`}
          </div>
          {project.reviewer && (
            <div className="flex items-center gap-2">
              <GraduationCap className="h-4 w-4 text-accent" /> Revisor: {project.reviewer.profile?.fullName ?? project.reviewer.username}
            </div>
          )}
          <div className="pt-1">
            <div className="mb-1.5 text-xs font-bold uppercase tracking-wider text-muted-foreground">Tecnologías</div>
            <div className="flex flex-wrap gap-1.5">
              {(project.technologies ?? []).map((technology) => <Badge key={technology.id} variant="accent">{technology.name}</Badge>)}
            </div>
          </div>
        ...
        <div className="rounded-xl border border-border bg-card p-5 shadow-sm">
          ...
          <div className="space-y-2">
            {(project.members ?? []).map((member, index) => {
              const username = member.user?.username ?? `miembro-${index + 1}`;
              return (
                <Link key={`${username}-${index}`} href={`/perfil/${username}`} className="flex items-center gap-3 rounded-lg p-2 transition hover:bg-secondary">
                  <Avatar src={member.user?.profile?.avatarUrl} name={member.user?.profile?.fullName ?? username} className="h-8 w-8" />
                  <div>
                    <div className="text-sm font-bold">{member.user?.profile?.fullName ?? username}</div>
                    {member.roleInProject && <div className="text-xs text-muted-foreground">{member.roleInProject}</div>}
                  </div>
                </Link>
              );
            })}
